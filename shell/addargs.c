	...
	)
{
	...
	*aptr++ = (uint32)NULL;
	...
	for (search = (uint32 *)prptr->prstkptr;
	     search < (uint32 *)prptr->prkstkbase; search++) {

		/* If found, replace with the address of the args vector*/
		/* 	in both kernel stack and user stack.		*/

		if (*search == (uint32)dummy) {
			*search = (uint32)argloc;
			*(prptr->prustkbase + ((char *)search - prptr->prkstkbase)) = (uint32)argloc;
			restore(mask);
			return OK;
		}
	}
