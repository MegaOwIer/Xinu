...
/*------------------------------------------------------------------------
 *  kbdhandler  -  Handle an interrupt for a keyboard
 *------------------------------------------------------------------------
 */
void	kbdhandler(void)
{
	static	uint32	shift;
	static	uint8	*charcode[] = {
	...
	if (c == TY_BACKSP || c == TY_BACKSP2) {
		...
		*kbdcb.tyitail++ = c;
		...
		return;
	} else if (c == TY_TAB || c == TY_ESC) {
		/* Ignore tab and ESC from keyboard input */
		return;
	}
